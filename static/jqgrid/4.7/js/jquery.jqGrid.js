        ...
        return this.each(function () {
            ...
            var sortkeys = ["shiftKey", "altKey", "ctrlKey"],
                ...
                addLocalData = function () {
                    ...
                    /*                    if (ts.p.treeGrid) {
                                            var stype = cmtypes[st] && cmtypes[st].stype ? cmtypes[st].stype : 'text'
                                            var srcfmt = cmtypes[st] && cmtypes[st].srcfmt?cmtypes[st].srcfmt: ''
                                            $(ts).jqGrid("SortTree", st, ts.p.sortorder, stype, srcfmt);
                                            return;
                                        }*/
                    var compareFnMap = {
    ...
    $.jgrid.extend({
        editCell: function (iRow, iCol, ed) {
            return this.each(function () {
                ...
                if (cm.editable === true && ed === true && !cc.hasClass("not-editable-cell") && !$($t.rows[iRow]).hasClass("not-editable-row")) {
                    if (parseInt($t.p.iCol, 10) >= 0 && parseInt($t.p.iRow, 10) >= 0) {
                        $("td:eq(" + $t.p.iCol + ")", $t.rows[$t.p.iRow]).removeClass("edit-cell ui-state-highlight");
                        $($t.rows[$t.p.iRow]).removeClass("selected-row ui-state-hover");
                    }
                    ...
                    $("input",cc).attr('autocomplete','off')
                    $("input, select, textarea", cc).bind("keydown", function (e) {
                        ...
                        if (e.keyCode === 40) {
                            if (!$t.grid.hDiv.loading) {
                                if($t.p.data && iRow<$t.p.data.length){
